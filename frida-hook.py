...
Hooks the functions SSL_read() and SSL_write() in a given process and sends the decrypted data to the console.
'''
_FRIDA_SCRIPT = r"""
 /**
 ...
 function initializeGlobals()
 {
   ...
   for (var i = 0; i < exps.length; i++)
   {
     ...
     for (var j = 0; j < names.length; j++)
     {
       ...
       if (matches.length == 0)
       {
         console.error("Could not find " + lib + "!" + name); 
         // Improved error reporting
         return; // Stop if a function is not found
       }
       else if (matches.length != 1)
       {
         ...
         if (!duplicates_only)
         {
           console.error("More than one match found for " + lib + "!" + name + ": " +
                s);
           return; // Stop on multiple matches
         }
       }
...
# frida hooking
device = frida.get_usb_device()  # Updated to get_usb_device()
